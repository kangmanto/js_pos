...
SoldReturnAction::SoldReturnAction() : ServerAction("solditemreturns", "id") {
...
void SoldReturnAction::afterInsert(const QVariantMap &data) {
    ...
    ItemUtil util(mDb);
    util.insertStock(barcode, QObject::tr("Sold Return : %1").arg(barcode), STOCK_CARD_TYPE::SOLD_RETURN,
                     data["count"].toFloat() - data["item_returned"].toFloat(), 0, data["id"].toInt());
    if (status == PURCHASE_RETURN_STATUS::RETURNED) {
        QVariantMap d{{"date", data["return_date"]},
                      {"number", data["barcode"]},
                      {"type", TRANSACTION_TYPE::EXPENSE},
                      {"link_id", data["id"]},
                      {"link_type", TRANSACTION_LINK_TYPE::SOLD_RETURN},
                      {"transaction_total", -data["money_returned"].toDouble()},
                      {"user_id", data["user_id"]},
                      {"machine_id", data["machine_id"]},
                      {"bank_id", data["bank_id"]},
                      {"money_total", -data["money_returned"].toDouble()},
...
void SoldReturnAction::afterUpdate(const QVariantMap &oldData, const QVariantMap &newData) {
    ...
    if (oldStatus != newStatus) {
        if (newStatus == PURCHASE_RETURN_STATUS::UNRETURN) {
            addCount -= oldRet;
            mDb->where("link_type =", TRANSACTION_LINK_TYPE::SOLD_RETURN)
            ...
            QVariantMap d{{"date", newData["return_date"]},
                          ...
                          {"link_id", newData["id"]},
                          {"link_type", TRANSACTION_LINK_TYPE::SOLD_RETURN},
                          {"transaction_total", -newData["money_returned"].toDouble()},
                          {"user_id", newData["user_id"]},
                          {"machine_id", newData["machine_id"]},
                          {"bank_id", newData["bank_id"]},
                          {"money_total", -newData["money_returned"].toDouble()},
