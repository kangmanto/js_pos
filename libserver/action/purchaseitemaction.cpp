...
PurchaseItemAction::PurchaseItemAction() : ServerAction(QStringLiteral("purchaseitems"), QStringLiteral("id")) {
...
LibG::Message PurchaseItemAction::insert(LibG::Message *msg) {
    ...
    DbResult res = mDb->where("barcode = ", msg->data("barcode"))
                       ->where("purchase_id = ", msg->data("purchase_id"))
                       ->get(mTableName);
    if (!res.isEmpty()) {
    ...
    if (!mDb->insert(mTableName, msg->data())) {
        ...
        mDb->exec(QString("UPDATE items SET buy_price = %1 WHERE barcode = '%2'")
                      .arg(QString::number(buyPrice))
                      .arg(barcode));
        if (editSellPrice) {
            mDb->exec(
                QString(
                    "UPDATE sellprices SET price = %1, discount = %2, final = %3 WHERE barcode = '%4' AND count = %5")
                    .arg(sellPrice)
                    .arg(discount)
                    .arg(final)
                    .arg(barcode)
...
LibG::Message PurchaseItemAction::update(LibG::Message *msg) {
    ...
    if (!mDb->update(mTableName, d)) {
        ...
        mDb->exec(
            QString("UPDATE items SET buy_price = %2 WHERE barcode = %3").arg(buyPrice).arg(old["barcode"].toString()));
        if (editSellPrice) {
            mDb->exec(
                QString("UPDATE sellprices SET price = %1, discount = %2, final = %3 WHERE barcode = %4 AND count = %5")
                    .arg(sellPrice)
                    .arg(discount)
                    .arg(final)
                    .arg(old["barcode"].toString())
                    .arg(count));
        }
        updatePurchaseTotal(pid);
        ItemUtil util(mDb);
        util.updateStockCardCount(res.first()["barcode"].toString(), n["count"].toFloat(), STOCK_CARD_TYPE::PURCHASE,
                                  msg->data("id").toInt(), true);
...
void PurchaseItemAction::selectAndJoin() {
    mDb->table(mTableName)
        ->select("purchaseitems.*, items.name, items.buy_price, \
...
void PurchaseItemAction::updatePurchaseTotal(int purchaseid) {
    ...
    mDb->where("id = ", purchaseid)
        ->update("purchases", QVariantMap{{"total", total}, {"discount", discount}, {"final", total - discount}});
