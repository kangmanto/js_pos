...
ServerAction::ServerAction(const QString &tableName, const QString idfield)
    : mDb(Db::createInstance()), mTableName(tableName), mIdField(idfield) {
    ...
    mFunctionMap.insert(MSG_COMMAND::RESTORE_DELETED,
                        std::bind(&ServerAction::restoreDelete, this, std::placeholders::_1));
}

ServerAction::~ServerAction() { delete mDb; }
...
QMap<QString, QString> ServerAction::fieldMap() const { return QMap<QString, QString>(); }

bool ServerAction::hasFlag(int flag) { return (mFlag & flag) != 0; }
