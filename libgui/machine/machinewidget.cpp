...
void MachineWidget::editClicked(const QModelIndex &index) {
    ...
    dialog.exec();
    mTableWidget->getModel()->refresh();
    /*const QString &name = QInputDialog::getText(this, tr("Edit machine"), tr("Input machine name"), QLineEdit::Normal,
    item->data("name").toString()); if(!name.isEmpty() && name.compare(item->data("name").toString())) { Message
    msg(MSG_TYPE::MACHINE, MSG_COMMAND::UPDATE); msg.addData("id", item->id); msg.addData("data", QVariantMap{{"name",
