...
void CategoryTreeWidget::populateData(QTreeWidgetItem *item, QList<CategoryData> &data, int pos) {
    ...
    CategoryData d(item->data(0, Qt::UserRole).toInt(), 0, name, item->data(1, Qt::DisplayRole).toString());
    data.append(d);
    for (int i = 0; i < item->childCount(); i++) {
        if (item->childCount() > 0) {
            ...
            CategoryData d1(item->child(i)->data(0, Qt::UserRole).toInt(), 0, n,
                            item->child(i)->data(1, Qt::DisplayRole).toString());
            data.append(d1);
        }
    }
}
