...
TableModel::TableModel(QObject *parent, bool useStandartHeader)
    : QAbstractTableModel(parent), mUseStandartHeader(useStandartHeader) {
    mIsLoaded = false;
    mNumRow = 0;
    mQuery.setLimit(mPerPage);
}

int TableModel::rowCount(const QModelIndex & /*parent*/) const { return mRowCount; }

int TableModel::columnCount(const QModelIndex & /*parent*/) const { return mHeaders.size(); }
...
void TableModel::addColumn(const QString &key, const QString &title, const int &align,
                           std::function<QVariant(TableItem *, const QString &)> formater) {
...
void TableModel::addHeaderFilter(const QString &key, HeaderFilter filter) { mHeaderFilter.insert(key, filter); }

void TableModel::setFilter(const QString &key, int type, const QVariant &value) { mQuery.setFilter(key, type, value); }

void TableModel::clearFilter() { mQuery.clearFilter(); }

void TableModel::setSort(const QString &sort) { mQuery.setSort(sort); }
...
void TableModel::slotPerPageCount(int value) { setPerPageCount(value, true); }
...
void TableModel::filterChanged(int index, const QVariant &value) {
    ...
    if (filter.compare == FilterLike) {
        ...
        if (value.canConvert(QVariant::Map)) {
            ...
            if (!mDateTimeISO) {
                mQuery.setFilter("0$" + key, COMPARE::GREATER_EQUAL,
                                 map["start"].toDateTime().toString("yyyy-MM-dd hh:mm:ss"));
                mQuery.setFilter("1$" + key, COMPARE::LESS_EQUAL,
                                 map["end"].toDateTime().toString("yyyy-MM-dd hh:mm:ss"));
            } else {
...
void TableModel::readData(LibG::Message *msg) {
    ...
    if (!mIsLoaded) {
        if (mNumRow != num) {
            mNumRow = num;
            if (num > 0 && num % mPerPage == 0)
                emit maxPageChanged(num / mPerPage);
            else if (num > 0)
                emit maxPageChanged(num / mPerPage + 1);
        }
        endResetModel();
