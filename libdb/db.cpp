...
Db::Db() : parent(NULL) { mDebug = DEBUG; }
...
bool Db::setDbSetting(const QString &host, int port, const QString &username, const QString &password,
...
void Db::setDatabaseType(const QString &db) { DBTYPE = db; }
...
DbResult Db::get(const QString &tableName) { return table(tableName)->exec(); }
...
bool Db::beginTransaction() { return getDatabase().transaction(); }

bool Db::commit() { return getDatabase().commit(); }

bool Db::roolback() { return getDatabase().rollback(); }

bool Db::isSQLite() { return DBTYPE.compare("SQLITE") == 0; }
...
bool Db::init(const QString &host, int port, const QString &username, const QString &password, const QString &dbname,
              bool checkDBName, bool newConnection) {
